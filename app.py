...
def analyze_network_traffic():
    ...
    try:
        ...
        try:
            traffic_data = request.get_json(force=True)
        except Exception:
            return jsonify({'error': 'Invalid JSON'}), 400
        
        if not traffic_data:
            return jsonify({'result': 'No data provided', 'risk_score': 0}), 200
...
def get_events():
    ...
    try:
        ...
        # For test scenarios, return empty list when no database is available
