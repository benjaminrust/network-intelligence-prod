...
export class AIService {
    ...
    async performInference(appName, model, inputData) {
        try {
    ...
    async batchInference(appName, requests) {
        try {
                ...
                content: [
                    {
                        ...
                        text: `Batch AI Inference for ${appName}:\n\n` +
                            ...
                            `Results:\n` +
                            `${result.results.map((req) => `Request ${req.request_id}: ${req.inference_type}\n` +
                ...
                content: [
                    {
                        ...
                        text: `Available AI Models:\n\n` +
                            `${models.models.map((model) => `${model.name} (${model.id})\n` +
    ...
    async getInferenceHistory(appName, inferenceType) {
        try {
                ...
                content: [
                    {
                        ...
                        text: `Inference History for ${appName} (${inferenceType}):\n\n` +
                            `${history.map((entry) => `Time: ${entry.timestamp}\n` +
