...
export class NetworkIntelligenceService {
    ...
    async analyzeTraffic(appName, trafficData) {
        try {
                ...
                content: [
                    {
                        ...
                        text: `Network Traffic Analysis for ${appName}:\n\n` +
                            ...
                            `Threats:\n${analysis.threats_detected.map((threat) => `- ${threat}`).join('\n')}\n\n` +
    ...
    async getNetworkStatus(appName) {
        try {
    ...
    async getSecurityAlerts(appName) {
        try {
                ...
                content: [
                    {
                        ...
                        text: `Security Alerts for ${appName}:\n\n` +
                            ...
                            `${alerts.map((alert) => `[${alert.severity.toUpperCase()}] ${alert.type}: ${alert.description}\n` +
    ...
    async getThreatIntelligence(appName) {
        try {
                ...
                content: [
                    {
                        ...
                        text: `Threat Intelligence for ${appName}:\n\n` +
                            ...
                            `${indicators.indicators.map((indicator) => `[${indicator.confidence.toUpperCase()}] ${indicator.type}: ${indicator.value}\n` +
    ...
    async getAnalytics(appName, metricName) {
        try {
                ...
                content: [
                    {
                        ...
                        text: `Analytics for ${appName}:\n\n` +
                            ...
                            `${analytics.metrics.map((metric) => `${metric.metric_name}: ${metric.metric_value} ${metric.metric_unit}\n` +
...
//# sourceMappingURL=network-intelligence.js.map