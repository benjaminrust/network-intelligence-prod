...
export class HerokuService {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${apps.length} Heroku apps:\n\n${apps.map((app) => `- ${app.name} (${app.region?.name || 'unknown'}) - ${app.state || 'unknown'}`).join('\n')}`
                    }
                ]
            };
        }
    ...
    async deployApp(appName, environment) {
        try {
    ...
    async scaleApp(appName, dynoType, quantity, size) {
        try {
    ...
    async queryDatabase(appName, query) {
        try {
    ...
    async getPipelineResource() {
        try {
            const { stdout } = await execAsync('heroku pipelines --json');
            const pipelines = JSON.parse(stdout);
            const networkIntelligencePipeline = pipelines.find((p) => p.name === 'network-intelligence');
            return {
                contents: [
                    {
